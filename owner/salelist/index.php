        ...
        <div class="col-12">
          <div class="card mb-4">
            <div class="card-header pb-0 d-flex align-items-center justify-content-between mb-3">
              <div class="ms-md-auto pe-md-3 d-flex align-items-center flex-wrap gap-2">

                <!-- 🔎 ค้นหาชื่อสินค้า -->
                <div class="input-group mb-3" style="min-width: 320px;">
                ...
                <!-- 🗓️ กรองตามช่วงวันที่ใบเสร็จ -->
                <div class="input-group ms-2 mb-3" style="min-width: 360px;">
                  <span class="input-group-text">จาก</span>
                  <input type="date" class="form-control" v-model="dateFrom" @change="goToFirstPage" />
                  <span class="input-group-text">ถึง</span>
                  <input type="date" class="form-control" v-model="dateTo" @change="goToFirstPage" />
                  <button class="btn btn-outline-secondary" type="button" @click="resetDateFilter">ล้าง</button>
                </div>
            ...
            <div class="card-body px-2 pt-0 pb-2">
                ...
                <table class="table align-items-center mb-0">
                  ...
                  <tbody>
                    ...
                    <tr v-if="!paginatedReceipts.length">
                      <td colspan="6" class="text-center text-muted py-4">ไม่พบข้อมูลในช่วงวันที่ที่เลือก</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div class="row d-flex d-sm-flex d-md-none d-lg-none justify-content-center" style="width: 100%;">
                ...
                <div v-if="!paginatedReceipts.length" class="text-center text-muted py-4">ไม่พบข้อมูลในช่วงวันที่ที่เลือก</div>
              </div>
            </div>
  ...
  <script type="module">
    ...
    createApp({
      ...
      computed: {
        ...
        /* ✅ กรองใบเสร็จตามช่วงวันที่ */
        filteredReceipts() {
          if (!this.dateFrom && !this.dateTo) return this.receipts;

          const from = this.dateFrom ? dayjs(this.dateFrom).startOf('day') : null;
          const to   = this.dateTo   ? dayjs(this.dateTo).endOf('day')   : null;
        ...
        totalPages() {
          const pages = Math.ceil(this.filteredReceipts.length / this.perPage);
      ...
      methods: {
        ...
        goToFirstPage() {
          // ถ้าเลือก dateFrom > dateTo ให้สลับค่า
          if (this.dateFrom && this.dateTo) {
            const f = dayjs(this.dateFrom);
            const t = dayjs(this.dateTo);
            if (f.isAfter(t)) {
        ...
        async getReceipts() {
          try {
            ...
            if (res.data.status) {
              ...
              // เมื่อโหลดใหม่แล้วคงหน้าปัจจุบันไว้ แต่ถ้าหน้าเกินจำนวนใหม่ ให้เด้งไปหน้าแรก
              if ((this.currentPage - 1) * this.perPage >= this.filteredReceipts.length) {
                this.currentPage = 1;
              }
            }
          } catch (err) {
        ...
        getType: async function() {
          try {
            ...
            const response = await axios.post(url, { post: 'get_type' }, config);
        ...
        loadWarehouses() {
          axios.post('../../api/', { post: 'get_warehouses_fproduct' })
            ...
            .catch(() => alert('โหลดคลังสินค้าล้มเหลว'));
        },
