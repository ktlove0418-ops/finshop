  ...
  <script type="module">
    ...
    createApp({
      ...
      computed: {
        ...
        //   return Object.entries(this.cashBreakdown).reduce((t, [denom, count]) => t + (parseInt(denom) || 0) * (count || 0), 0);
        // },
        totalCashFromBreakdown() {
      ...
      mounted() {
        ...
        this.loadWarehouses().then(() => {
          // ดึงคลังล่าสุดที่เคยเลือก
          const lastWh = localStorage.getItem('LAST_WAREHOUSE_ID');
          if (lastWh) {
            this.warehouseId = lastWh;
            this.onChangeWarehouse(); // โหลดสินค้า + ตะกร้า เพื่อให้ badge แสดงทันที
          }
        });
