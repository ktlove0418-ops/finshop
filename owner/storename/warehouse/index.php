...
<body class="g-sidenav-show bg-gray-100">
                    ...
                    <tbody>
                      <tr v-for="(item, index) in paginatedProducts" :key="index">
                        ...
                        <td class="align-middle pt-3 text-center">
                          <button v-if="edtu && edId == item.id" class="btn btn-sm px-4 mx-1 btn-success"
                            @click="saveEditUnits(item)">
                            บันทึก
                          </button>
                          <button v-else class="btn btn-sm px-2 mx-2 btn-info"
                            @click="editUnits(item)">
                            จัดการจำนวน
                          </button>

                          <button v-if="edtu && edId == item.id" class="btn btn-sm px-2 mx-1 btn-danger"
                            @click="cancelUnits()">
                            เลิกทำ
                          </button>
                          <button v-else class="btn btn-sm px-2 mx-1 btn-secondary"
                            @click="delModal(item)">
                            ตั้งสูงสุด
                          </button>

                          <button class="btn btn-sm px-2 mx-1 btn-warning"
                            @click="openLogs(item)"
      ...
      <div class="modal fade" id="logModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
          <div class="modal-content">
            <div class="modal-header d-flex align-items-center gap-2 flex-wrap">
              <h5 class="modal-title mb-0">
                ประวัติการแก้ไขสินค้า: {{ logTarget?.product_name || '-' }}
              </h5>
              ...
              <div class="ms-auto d-flex align-items-center gap-2">
                <input
                  ...
                  @change="reloadLogsByDate" />
                <button  style="width: 220px; padding:2px" class="btn btn-sm btn-outline-secondary" type="button" @click="clearLogDate">
                  ล้างวันที่
                </button>
  ...
  <script type="module">
    ...
    createApp({
      ...
      methods: {
        async reloadLogsByDate() {
          // ถ้ายังไม่เลือกสินค้าเป้าหมายก็ไม่ต้องโหลด
          if (!this.logTarget?.pw_id) return;
          await this.openLogs(this.logTarget); // reuse ฟังก์ชันเดิมให้โหลดตาม date
        },

        clearLogDate() {
          this.logFilterDate = '';
          if (this.logTarget?.pw_id) this.openLogs(this.logTarget);
        },
        getBack() {
          window.location = "../top";
        },
          ...
          try {
            ...
            if (profileRaw) {
              try {
                ...
                user_role = parsed?.data?.position || parsed?.position || "owner";
              } catch (e) {}
            }
